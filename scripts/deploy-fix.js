...
async function main() {
  ...
  let vault = await deployUtils.deploy("SerpentShields.sol", deployer.address);
  // let vault = await deployUtils.attach("SerpentShields.sol", deployer.address);
  await deployUtils.Tx(vault.init(registry.address, guardian.address, managerProxy.address), "Init vault");

  let factory = await deployUtils.attach("SerpentShieldsFactory");
